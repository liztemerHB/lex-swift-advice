...
import { ArrowLeft, Loader2, Upload, X, Camera, FileText, Save } from "lucide-react";
...
const LawyerProfileEdit = () => {
  ...
  const save = async () => {
    ...
    if (isComplete) navigate("/lawyer");
  };

  const back = role === "admin" ? "/admin" : "/lawyer";
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-app flex-col bg-background">
      <header className="sticky top-0 z-10 flex items-center gap-3 border-b border-border bg-background/95 px-3 py-3 backdrop-blur">
        <Link to={back} className="rounded-md p-1.5 text-muted-foreground hover:bg-muted">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <p className="flex-1 text-sm font-semibold text-foreground">Профиль юриста</p>
      ...
      <div className="flex-1 space-y-5 px-4 py-5">
        {!isComplete && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
            Заполните обязательные поля, чтобы профиль стал виден клиентам.
          </div>
        )}
        ...
        <section className="space-y-3">
          ...
          <Field label="Высшее юридическое образование">
            <Textarea
              ...
              placeholder="МГУ им. М.В. Ломоносова, юридический факультет, 2015"
            />
          </Field>
          <Field label="Дополнительное образование, курсы, степени">
            <Textarea
              rows={3}
              value={profile.additional_education ?? ""}
              onChange={(e) => setProfile({ ...profile, additional_education: e.target.value })}
              placeholder="Магистр частного права, РШЧП, 2018; курсы повышения квалификации…"
            />
          </Field>
        </section>
        ...
        <section className="space-y-3">
          ...
          <Field label="Опыт работы (места работы, должности, годы)">
            <Textarea
              rows={4}
              value={profile.work_experience ?? ""}
              onChange={(e) => setProfile({ ...profile, work_experience: e.target.value })}
              placeholder="2018–2023 — старший юрист, КА «Право»; 2023 — настоящее время — частная практика"
            />
          </Field>
        </section>
        ...
        {/* Practice */}
        <section className="space-y-2">
          <h3 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Практика — какие споры ведёте *</h3>
        ...
        <section className="space-y-3 rounded-xl border border-border p-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">У вас есть удостоверение адвоката</p>
            </div>
            <Switch
        ...
        <section className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="Краткий рассказ о вашем подходе к работе, ключевых кейсах и сильных сторонах"
            maxLength={1000}
          />
        </section>
...
const DocRow = ({ path, onRemove }: { path: string; onRemove: () => void }) => {
  ...
  return (
    <div className="flex items-center justify-between rounded-lg border border-border bg-background px-3 py-2 text-xs">
      <a href={url ?? "#"} target="_blank" rel="noreferrer" className="flex min-w-0 items-center gap-2 text-foreground hover:underline">
